...
public class MyWidgetProvider extends AppWidgetProvider {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
		...
		for(int appWidgetId : appWidgetIds) {
			...
			if( sharedPreferences.getBoolean(MainActivity_CameraOpen.getShowWhenLockedPreferenceKey(), true) ) {
		    	if( MyDebug.LOG )
		    		Log.d(TAG, "do show above lock screen");
	            Intent intent = new Intent(context, MyWidgetProvider.class);
	            intent.setAction("com.embaby.camerastudio.LAUNCH_OPEN_CAMERA");
	            pendingIntent = PendingIntent.getBroadcast(context, 0, intent, 0);
			}
			else*/ {
		    	/*if( MyDebug.LOG )
		    		Log.d(TAG, "don't show above lock screen");*/
	            Intent intent = new Intent(context, MainActivity_CameraOpen.class);
	...
	public void onReceive(Context context, Intent intent) {
	    ...
	    if (intent.getAction().equals("com.embaby.camerastudio.LAUNCH_OPEN_CAMERA")) {
	    	if( MyDebug.LOG )
	    		Log.d(TAG, "Launching MainActivity_CameraOpen");
	        final Intent activity = new Intent(context, MainActivity_CameraOpen.class);
