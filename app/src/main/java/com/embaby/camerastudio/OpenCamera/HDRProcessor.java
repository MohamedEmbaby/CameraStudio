...
import com.embaby.camerastudio.OpenCamera.HdrScripts.ScriptC_align_mtb;
import com.embaby.camerastudio.OpenCamera.HdrScripts.ScriptC_create_mtb;
import com.embaby.camerastudio.OpenCamera.HdrScripts.ScriptC_histogram_adjust;
import com.embaby.camerastudio.OpenCamera.HdrScripts.ScriptC_histogram_compute;
import com.embaby.camerastudio.OpenCamera.HdrScripts.ScriptC_process_hdr;
...
public class HDRProcessor {
	...
	public void onDestroy() {
		...
		if( rs != null ) {
			...
			try {
				rs.destroy(); // on Android M onwards this is a NOP - instead we call RenderScript.releaseAllContexts(); in MainActivity_CameraOpen.onDestroy()
			}
	...
	private static class ResponseFunction {
		...
		ResponseFunction(Context context, int id, List<Double> x_samples, List<Double> y_samples, List<Double> weights) {
			...
			if( MyDebug.LOG ) {
				...
				File file = new File(Environment.getExternalStorageDirectory().getPath() + "/com.embaby.camerastudio.hdr_samples_" + id + ".csv");
				if( file.exists() ) {
					if( !file.delete() ) {
		...
		for(int i=0;i<allocations.length;i++) {
			...
			/*if( MyDebug.LOG ) {
				...
				try {
					...
					outputStream.close();
					MainActivity_CameraOpen mActivity = (MainActivity_CameraOpen) context;
