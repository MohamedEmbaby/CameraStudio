...
public class MainUI {
	...
	private static int PREVIEW_WIDTH_CURRENT = 0;
	...
	public MainUI(MainActivity_CameraOpen main_activity) {
		if( MyDebug.LOG )
			Log.d(TAG, "MainUI");
...
//		this.setIcon(R.id.share);
    ...
    private void layoutUI(boolean popup_container_only) {
		...
		if( !popup_container_only )
		{
			...
			ImageView imageView = (ImageView) main_activity.findViewById(R.id.image_csdk);
...
//			} else {
//				imageView.getLayoutParams().width = Methods.getScreenWidth();
//				imageView.getLayoutParams().height = Methods.getScreenHeight();
//			}
			...
			view = main_activity.findViewById(R.id.edit);
			layoutParams = (RelativeLayout.LayoutParams)view.getLayoutParams();
			layoutParams.addRule(align_parent_top, RelativeLayout.TRUE);
			layoutParams.addRule(align_parent_bottom, 0);
			layoutParams.addRule(left_of, R.id.share);
			layoutParams.addRule(right_of, 0);
			view.setLayoutParams(layoutParams);
			setViewRotation(view, ui_rotation);
...

		{
			...
			layoutParams.addRule(RelativeLayout.BELOW, R.id.settings);
    ...
    public void setTakePhotoIcon() {
		...
		if( main_activity.getPreview() != null ) {
			...
			ImageButton switchVideoButton = (ImageButton)main_activity.findViewById(R.id.switch_video);
			if( main_activity.getPreview().isVideo() ) {
				if( MyDebug.LOG )
					Log.d(TAG, "set icon to video");
				switchVideoButton.setImageDrawable(main_activity.getResources().getDrawable(R.drawable.ic_camera_phote_white_24dp));
				resource = main_activity.getPreview().isVideoRecording() ? R.drawable.uni_red : R.drawable.uni;
				...
				switchVideoButton.setImageDrawable(main_activity.getResources().getDrawable(R.drawable.ic_camera_video_white_24dp));
				resource = R.drawable.uni;
...
//			int cameraId = Methods.getDefaultsInt("camera_id", main_activity.getApplicationContext());
		...
		if( is_open ) {
			...
			if( main_activity.getPreview().usingCamera2API() && !iso_value.equals("auto") ) {
				...
				if( main_activity.getPreview().supportsExposures() ) {
					...
					View seek_bar_zoom = main_activity.findViewById(R.id.exposure_seekbar_zoom);
    ...
    public void closePopup() {
		...
		if( popupIsOpen() ) {
			 ...
			 *     MainActivity_CameraOpen.updateForSettings(), but doing so makes the popup close when checking photo or video resolutions!
...
//			popup_container.setAlpha(0.9f);
			popup_container.setBackground(main_activity.getResources().getDrawable(R.drawable.border_rectangle_black_transparent));
		}
